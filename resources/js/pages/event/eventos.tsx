            ...
            <div className="flex h-full flex-1 flex-col gap-5 overflow-x-auto rounded-xl p-4">
                ...
                <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-admin-600 via-admin-700 to-admin-900 p-4 text-white shadow-xl">
                    ...
                    <div className="relative flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div>
                            ...
                            <p className="text-sm text-admin-100/80 mt-0.5">{stats.total} evento{stats.total !== 1 ? 's' : ''} registrado{stats.total !== 1 ? 's' : ''}</p>
                        </div>
                        <button onClick={() => { setEditEvento(null); setModalOpen(true); }}
                            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white text-admin-700 font-bold text-sm hover:bg-admin-50 cursor-pointer shadow-lg shrink-0">
                ...
                <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-4 space-y-3">
                    <div className="flex flex-wrap gap-3 items-center">
                        ...
                        <select value={filtEstado} onChange={e => resetPageAnd(setFiltEstado)(e.target.value)} className="px-3 py-2.5 text-sm rounded-xl border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 outline-none focus:border-admin-400 cursor-pointer">
                            <option value="">Todos los estados</option>
                            {estados.map(e => <option key={e.idestado_event} value={e.idestado_event}>{e.nom_estado_event}</option>)}
                        </select>

                        <select value={filtModalidad} onChange={e => resetPageAnd(setFiltModalidad)(e.target.value)} className="px-3 py-2.5 text-sm rounded-xl border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 outline-none focus:border-admin-400 cursor-pointer">
                            <option value="">Todas las modalidades</option>
                            {modalidades.map(m => <option key={m.idmodalidad} value={m.idmodalidad}>{m.nom_modalidad}</option>)}
                        </select>

                        <select value={filtTipo} onChange={e => resetPageAnd(setFiltTipo)(e.target.value)} className="px-3 py-2.5 text-sm rounded-xl border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 outline-none focus:border-admin-400 cursor-pointer">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        <input type="date" value={filtDesde} onChange={e => resetPageAnd(setFiltDesde)(e.target.value)} className="px-2.5 py-1.5 text-xs rounded-lg border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 outline-none focus:border-admin-400" />
                        <span className="text-xs text-zinc-400">hasta</span>
                        <input type="date" value={filtHasta} onChange={e => resetPageAnd(setFiltHasta)(e.target.value)} className="px-2.5 py-1.5 text-xs rounded-lg border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 outline-none focus:border-admin-400" />
                ...
                {eventosFilt.length === 0 && (
                    <div className="flex flex-col items-center justify-center py-16 gap-3 bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800">
                        ...
                        {!hayFiltros && (
                            <button onClick={() => setModalOpen(true)} className="mt-1 flex items-center gap-1.5 px-4 py-2 rounded-xl bg-admin-600 text-white text-sm font-bold cursor-pointer hover:bg-admin-700">
                ...
                {eventosFilt.length > 0 && (viewMode === 'grid' ? true : true) && (
                    <div className={`${viewMode === 'grid' ? 'grid' : 'grid md:hidden'} grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4`}>
                        {eventosFilt.map(ev => {
                            ...
                            return (
                                <div key={ev.idevento}
                                    className="group bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden hover:shadow-xl hover:border-admin-300 dark:hover:border-admin-700 transition-all duration-200 flex flex-col">

                                    {/* Imagen */}
                                    <div className="relative h-36 bg-gradient-to-br from-admin-100 to-admin-200 dark:from-admin-950/40 dark:to-admin-900/40">
                                        {ev.image_url
                                    ...
                                    <div className="p-4 flex flex-col gap-2.5 flex-1">
                                        <div>
                                            <p className="text-[10px] font-bold text-admin-500 dark:text-admin-400 uppercase tracking-wider">{ev.nom_tipo_evento}</p>
                                        ...
                                        <div className="flex items-center justify-between mt-auto pt-2 border-t border-zinc-100 dark:border-zinc-800">
                                            ...
                                            {ev.link_form_inscripcion && (
                                                <button onClick={() => copiarLink(ev.link_form_inscripcion!)} title="Copiar link de inscripción"
                                                    className="flex items-center gap-1 text-[10px] font-bold text-admin-600 dark:text-admin-400 hover:underline cursor-pointer">
                        ...
                        {eventosPag.map(ev => {
                            ...
                            return (
                                <div key={ev.idevento}
                                    ...
                                    <div className="relative h-32 bg-gradient-to-br from-admin-100 to-admin-200 dark:from-admin-950/40 dark:to-admin-900/40">
                                        {ev.image_url ? <img src={ev.image_url} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-admin-300"><Ico.Image /></div>}
                                        <span className={`absolute top-2.5 left-2.5 inline-flex items-center gap-1.5 text-[10px] font-black px-2.5 py-1 rounded-full ${ecfg.bg} ${ecfg.text} backdrop-blur-sm shadow`}>
                                            <span className={`w-1.5 h-1.5 rounded-full ${ecfg.dot} ${ev.nom_estado_event === 'En curso' ? 'animate-pulse' : ''}`} /> {ev.nom_estado_event}
                                        </span>
                                        <span className={`absolute top-2.5 right-2.5 inline-flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-full ${mcfg.bg} ${mcfg.text} backdrop-blur-sm shadow`}>
